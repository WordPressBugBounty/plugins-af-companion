...
if (!function_exists('af_companiion_display_starter_sites')) {
  function af_companiion_display_starter_sites()
  {
    $templatespare_old_version = '2.3.0';
    $af_companiion_templatespare_installed = af_companiion_get_plugin_file('templatespare');
    $af_companiion_templatespare_verison = '';

    if (!empty($af_companiion_templatespare_installed)) {
      $af_companiion_templatespare_info = get_plugin_data(WP_PLUGIN_DIR . '/' . $af_companiion_templatespare_installed);
      $af_companiion_templatespare_verison = $af_companiion_templatespare_info['Version'];
    }

    $af_companiion_templatespare_active = is_plugin_active('templatespare/templatespare.php');
    $install = [];
    $activate = [];
    if ($af_companiion_templatespare_installed == null) {
      $install = ['templatespare'];
    }

    if ($af_companiion_templatespare_active == false && $af_companiion_templatespare_installed != null) {
      $activate = ['templatespare'];
    }
    $plugin_update = 'false';
    if (!empty($af_companiion_templatespare_verison) && $af_companiion_templatespare_verison < $templatespare_old_version) {
      $plugin_update = 'true';
    }

    if (($af_companiion_templatespare_installed && $af_companiion_templatespare_active) && $plugin_update == 'false') { ?>
    ...
    <?php
      ...
      if (!empty($af_companiion_templatespare_verison) && $af_companiion_templatespare_active && $af_companiion_templatespare_verison < $templatespare_old_version) {
        $class = admin_url('plugins.php');

        $message = __('The Templatespare plugin should be updated to the latest version', 'af-companion');
      } else {
        $class = 'false';
        $message = __('Import a Starter Site, Personalize, and Live it in 3 Easy Steps!', 'af-companion');
      }
    ?>
      <div id="templatespare-plugin-install-activate" data-class=<?php echo $class; ?> current-theme="af-companion" install=<?php echo json_encode($install); ?> activate=<?php echo json_encode($activate); ?> data-plugin-page='af-companion' message='<?php echo $message; ?>' ispro=''></div>
<?php
    }
  }
}

function af_companiion_get_plugin_file($plugin_slug)
{
